...
public class CameraManager : MonoBehaviour
{
    ...
    private float shoulderOffsetAmout = 0.5f;
    ...
    private void BaseAction_OnAnyActionStarted(object sender, EventArgs empty)
    {
        switch (sender)
        {
            case ShootAction shootAction:
                PositionCameraToShoulder(shootAction);
                ShowActionCamera();
                break;
        }
    }

    private void PositionCameraToShoulder(ShootAction shootAction)
    {
        ...
        Vector3 shoulderOffset = Quaternion.Euler(0, 90, 0) * shootDirection * shoulderOffsetAmout;
